...
if output_type == "csv":
    class FootballGuysSpider(scrapy.Spider):
        ...
        def start_requests(self):
            # Open CSV file for writing and initialize headers
            if not os.path.exists(self.output_file):
                with open(self.output_file, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(['Team', 'Season', 'Week', 'Position', 'Player', 'SnapCount', 'SnapPercent'])
        ...
        def parse(self, response):
            ...
            for thead, tbody in zip(response.css(".table > thead"), response.css(".table > tbody")):
                ...
                for i, player in enumerate(players):
                    ...
                    # Append a flattened row to the list (Team, Week, Position, Player, SnapCount, SnapPercent)
